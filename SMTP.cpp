...
CSMTP::response_code CSMTP::response_table[] =
{
	...
	{ 334, "AUTH erro" },                             //AUTH_SUCCESS
	{ 235, "LOGIN FAILED" },                               //LOGIN_SUCCESS
...
BOOL CSMTP::SendMessage(CMailMessage * msg, CString user, CString password)
{
	ASSERT( msg != NULL );
	if( !m_bConnected )
	{
...
BOOL CSMTP::login(CString user, CString password){
	CBase64 encoder;
	CString sUser64, sPassword64, sLogin;
	sUser64 = encoder.Encode(user.GetBuffer(), user.GetLength());
	sPassword64 = encoder.Encode(password.GetBuffer(), password.GetLength());
	sUser64 = sUser64 + "\r\n";
	sPassword64 = sPassword64 + "\r\n";
	if (!m_bConnected)
	{
		m_sError = _T("Must be connected");
		return FALSE;
	}
	sLogin.Format("AUTH LOGIN\r\n");
	m_wsSMTPServer.Send((LPCTSTR)sLogin, sLogin.GetLength());
	if (!get_response(AUTH_SUCCESS))
		return FALSE;
	m_wsSMTPServer.Send((LPCTSTR)sUser64, sUser64.GetLength());
	
	if (!get_response(AUTH_SUCCESS))
		return FALSE;
	m_wsSMTPServer.Send((LPCTSTR)sPassword64, sPassword64.GetLength());
